...
class Database():
    ...
    def __init__(self):
        # 连接database
        self.conn = pymysql.connect(host="127.0.0.1", user="root",password="0212",database="chat_py",charset="utf8")
    ...
    #  未读消息已经发给上线的人了
    def updateUnreadMessage(self, id):
        sql = "UPDATE `message` set status = 1 WHERE target = '%d' AND status = 0" % (id)
        try:
            self.cursor.execute(sql)
            self.conn.commit()
            return True
